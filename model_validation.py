...
parser.add_argument("--balance_strategy", type=str, default="sqrt",
                    ...
                    help="增强平衡策略 (默认: sqrt)")
parser.add_argument("--balance_beta", type=float, default=0.99,
                    help="有效样本数策略的β参数 (仅 effective_number 使用)")
parser.add_argument("--target_count", type=int, default=None,
                    help="增强目标数 (仅 equal 策略使用)")
...
def _build_text(item):
    ...
    if literal:
        combined_text += " " + literal
    if pragmatic:
        combined_text += " " + pragmatic
    return combined_text


def _load_augmentation_module():
    """动态加载 data_augmentation_v3.py 模块"""
    aug_script = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "data_augmentation_v3.py"
    )
    if not os.path.exists(aug_script):
        aug_script = "data_augmentation_v3.py"
    spec = importlib.util.spec_from_file_location("data_augmentation_v3", aug_script)
...
def augment_fold_training_data(train_items, aug_module, fold_seed):
    ...
    texts = [_build_text(item) for item in augmented_train]
...
def main():
    ...
    if args.augment:
        try:
            ...
            print(f"\n[增强] 已加载增强模块，策略: {args.balance_strategy}")
        except Exception as e:
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(fold_splits):
        ...
        test_texts = [_build_text(item) for item in test_items]
        ...
        if aug_module is not None:
            ...
            print(f"[增强] 对第{fold_num}折训练集执行增强 "
                  f"(策略={args.balance_strategy}, seed={fold_seed})")
            try:
                ...
                train_texts = [_build_text(item) for item in train_original_items]
            ...
            if synthetic_data:
                for item in synthetic_data:
                    train_texts.append(_build_text(item))
    ...
    report = {
        ...
        "balance_strategy": args.balance_strategy if args.augment else None,
        "seed": args.seed,
        "results": summary,
    }
